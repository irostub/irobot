...
class HelpCommends(commands.Cog):
    ...
    async def helpCommand(self, ctx):
        embed=discord.Embed(title="~이로 도움말~",description="제게 시킬 수 있는 건 이런 것들이 있어요!"
        " 아직 공사중이라서 완벽히 지원하지 않는 명령어도 있답니다!\n`!이로 도움 [명령어]`", color=0xffff80)
        embed.set_author(name="안녕하세오...이로에오", icon_url = "https://i.imgur.com/h3qAGqV.png")
    ...
    async def helpCommand_reply(self, ctx):
        embed=discord.Embed(title="~대화 기능~",description="저랑 얘기를 하고 싶다구요? 혹시 제가 모르는게 있다면 학습기능을 통해 알려주세요 =w=\n`!이로 [짧은대사]`", color=0xffff80)
        embed.add_field(name="`자폭`", value="날..없앨꺼야..? 정말로..? <:TwT:625349630317821972>", inline=False)
    ...
    async def helpCommand_game(self, ctx):
        embed=discord.Embed(title="~채팅레벨 기능~",description="채팅을 쳐서 당신의 디코력을 뽐내보세요. 현자타임을 찾기 딱 좋아요!"
                                                           "후추 업데이트에서 채팅으로 얻은 경험치와 무기강화가 연동될꺼에요!\n`!이로 채팅 [명령어]`", color=0xffff80)
    ...
    async def helpCommand_forge(self, ctx):
        embed=discord.Embed(title="~강화 게임~",description="\n`!이로 강화 [명령어]`", color=0xffff80)
        embed.set_thumbnail(url="https://i.imgur.com/SXKirYF.png")
        embed.add_field(name="`실행`", value="강화를 하자구욧!", inline=False)
        embed.add_field(name="`상태`", value="자신의 강화 상태와 강화보호권, 확률증가권 개수를 보여줘요~", inline=False)
        embed.add_field(name="`강보`", value="강보권(강화보호권)를 사용할 수 있어요. 강화가 실패하기 전까지 보호해줘요", inline=False)
