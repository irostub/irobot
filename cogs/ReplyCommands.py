...
class ReplyCommands(commands.Cog):
    ...
    async def outd(self, ctx):
        await ctx.channel.send("이로봇 관짝 열고 퇴근합니다.")
        await self.client.logout()
        await self.client.close()

    @commands.command(name="물어")
    async def outds(self, ctx):
        await ctx.channel.send("으그으으...으르르..컁! 건들이지 마세요! 으르..")

    @commands.command(name="<Undefined>", aliases=["Undefined"])
    async def undefcall(self, ctx):
        await ctx.channel.send(
            "이걸 제게 말했다구요? 정말 할게 없었나보네오<:_tyeayea:619838526074126346> 보상으로 이곳에 당신이 원하는 특수대사를 넣어드리죠. 주인님께 말해주세요."
        )

    @commands.command(name="이로", aliases=["이로님", "이로야"])
    async def selfcall(self, ctx):
        await ctx.channel.send(
            "제 주인님은 iro_bound (입이 멋대로..움직여버렷:carrot::carrot:) 킹갓제네럴엠페러충무공마제스티하이퍼울트라판타스틱익스트림지니어스해요<:TwT:625349630317821972> "
        )
    ...
    async def destruction(self, ctx):
        ...
        author = ctx.author
        try:
            rems = await self.client.wait_for(
                "message", check=lambda message: ctx.author == author, timeout=10.0
            )
...
"""
                ...
                await ctx.channel.send(
                    embed=discord.Embed(
                        title="-__***SELF DESTRUCT SEQUENCE ENGAGED***__-",
                        color=0xFF0000,
                    )
                )
                await ctx.channel.send(
